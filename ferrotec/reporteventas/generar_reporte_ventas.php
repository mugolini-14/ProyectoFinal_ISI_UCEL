...
$dias_seleccionados = $interval->days + 1; // +1 para incluir el primer día


// Crear consulta SQL

$consulta_id_ventas_fecha = "SELECT id FROM ventas WHERE ventas_fecha >= '$fecha_inicio' AND ventas_fecha <= '$fecha_fin'";
...
if (!empty($id_ventas_fecha)) {
    ...
    $consulta_sum_cant_art = "SELECT SUM(vendet_cantidad) as suma_cant_art FROM ventas_detalle WHERE vendet_id_venta IN ($ids_lista_ventas_fecha)";

    $consulta_sum = "SELECT SUM(vendet_monto) as suma FROM ventas_detalle WHERE vendet_id_venta IN ($ids_lista_ventas_fecha)";

    $consulta_sum_cant_ventas = "SELECT COUNT(id) as suma_cant_ventas FROM ventas WHERE id IN ($ids_lista_ventas_fecha)";
    ...
    if (!empty($usuario)) {
        ...
        if ($resultado_id_usu->num_rows > 0) {
            ...
            $consulta_ventas_usu = "SELECT id FROM ventas WHERE ventas_id_usuario  = $id_usu";
            ...
            if (!empty($id_ventas_usu)) {
                ...
                // Buscamos los IDs de la tabla ventas_detalle de las ventas que se realizaron con el ID de la venta
                $consulta_vendet_usu = "SELECT id FROM ventas_detalle WHERE vendet_id_venta IN ($ids_ventas_usu_lista)";
                ...
                if (!empty($id_ventas_detalle_usu)) {
                    ...
                    $consulta_ven_id = "SELECT vendet_id_venta FROM ventas_detalle WHERE id IN ($ids_ventas_usu_lista)";
            ...
            echo "alert('Usuario no se encuentra en la Base de Datos');";
            exit; // Detiene la ejecución del script PHP
        }
    }

    if ($filtroOpcion == 'tipo') {
    ...
    if ($filtroOpcion == 'articulo') {
        ...
        if (!empty($id_articulos)) {
            ...
            //echo "IDs de articulos dentro de la lista de fecha: $ids_lista<br> que son los articulos $nombres_lista_art";
            // Buscamos los IDs de la tabla ventas_detalle de las ventas que se realizaron con el ID de la lista de artículos
            $consulta_vendet = "SELECT id FROM ventas_detalle WHERE vendet_id_art IN ($ids_lista_art)";
            ...
            if (!empty($id_ventas_detalle)) {
                ...
                $consulta_ven_id = "SELECT vendet_id_venta FROM ventas_detalle WHERE id IN ($ids_ventas_detalle_lista)";
                $resultado_ven_id = $conexion->query($consulta_ven_id);
                $id_ventas = [];
                if ($resultado_ven_id->num_rows > 0) {
                    while ($row = $resultado_ven_id->fetch_assoc()) {
