<?php
/*
    PHP:            generar_reporte_hsitorial_articulos.php
    Descripción:    Arma el PDF con los resultados del historial de Artículos
*/
    ...
    $json_string = $_POST['JSON-historial-proveedores'];
    ...
    foreach($filas as $row){
        ...
        $historial_prov_accion = $row['prov-accion'];
        $historial_modificado_por = $row['modificado-por'];
        $historial_proveedor = $row['nombre'];
        $historial_descripcion = $row['descripcion'];
        $historial_direccion = $row['direccion'];
        $historial_localidad = $row['localidad'];
        $historial_telefono1 = $row['telefono1'];
        $historial_telefono2 = $row['telefono2'];
        $historial_email = $row['email'];
        $historial_cuit = $row['cuit'];
        $historial_activo = $row['activo'];

        $html_filas .=
        "<tr>
            <td>{$historial_fecha_registro}</td>
            <td>{$historial_prov_accion}</td>
            <td>{$historial_modificado_por}</td>
            <td>{$historial_proveedor}</td>
            <td>{$historial_descripcion}</td>
            <td>{$historial_direccion}</td>
            <td>{$historial_localidad}</td>
            <td>{$historial_telefono1}</td>
            <td>{$historial_telefono2}</td>
            <td>{$historial_email}</td>
            <td>{$historial_cuit}</td>
    ...
    $pdf->SetTitle('Historial de Proveedores');
    $pdf->SetSubject('Historial de Proveedores');
    $pdf->SetKeywords('Ferrotec, Historiales, Proveedores');
    ...
    "<html>
        <body>
            ...
            <br>
                <p style='float:left;'> <b> Historial de Proveedores </b> </p>
    ...
    header('Content-Disposition: attachment; filename="consulta_historial_articulos_' . $fecha_reporte_archivo . '.pdf"');
    header('Cache-Control: max-age=0');

    // Grabar archivo con el reporte generado
    $pdf->Output('consulta_historial_proveedores_' . $fecha_reporte_archivo . '.pdf', 'D');
