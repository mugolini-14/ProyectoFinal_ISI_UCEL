<?php
/*
    PHP:            generar_reporte_historial_categorias.php
    Descripción:    Arma el PDF con los resultados del historial de Categorías
*/
    ...
    $json_string = $_POST['JSON-historial-categorias'];
    ...
    foreach($filas as $row){
        ...
        $historial_cat_accion = $row['cat-accion'];
        $historial_modificado_por = $row['modificado-por'];
        $historial_categoria = $row['categoria'];
        ...
        "<tr>
            ...
            <td>{$historial_cat_accion}</td>
            <td>{$historial_modificado_por}</td>
            <td>{$historial_categoria}</td>
    ...
    $pdf->SetTitle('Historial de Categorías');
    $pdf->SetSubject('Historial de Categorías');
    $pdf->SetKeywords('Ferrotec, Historiales, Categorías');
    ...
    "<html>
        <body>
            ...
            <br>
                <p style='float:left;'> <b> Historial de Categorías </b> </p>
    ...
    header('Content-Disposition: attachment; filename="consulta_historial_categorias_' . $fecha_reporte_archivo . '.pdf"');
    header('Cache-Control: max-age=0');

    // Grabar archivo con el reporte generado
    $pdf->Output('consulta_historial_categorias_' . $fecha_reporte_archivo . '.pdf', 'D');
