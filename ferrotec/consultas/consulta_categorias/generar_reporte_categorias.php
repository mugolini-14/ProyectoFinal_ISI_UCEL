<?php
/*
    PHP:            generar_reporte_categorias.php
    Descripción:    Arma el PDF con los resultados de la búsqueda de Categorías
*/
    ...
    $json_string = $_POST['JSON-categorias'];
    ...
    foreach($filas as $row){
        ...
        $categoria_dependedeltipo = $row['dependedeltipo'];
        $categoria_activo = $row['activo'];

        $html_filas .=
        "<tr>
            <td>{$categoria_nombre}</td>
            <td>{$categoria_descripcion}</td>
            <td>{$categoria_dependedeltipo}</td>
            <td>{$categoria_activo}</td>
        </tr>
        ";
    } 
    ...
    $pdf->SetTitle('Consulta de Categorias');
    $pdf->SetSubject('Consulta de Categorias');
    $pdf->SetKeywords('Ferrotec, Consulta, Categorias');
    ...
    "<html>
        <body>
            ...
            <br>
                <p style='float:left;'> <b> Consulta de Categorias </b> </p>
    ...
    header('Content-Disposition: attachment; filename="consulta_categorias_' . $fecha_reporte_archivo . '.pdf"');
    header('Cache-Control: max-age=0');

    // Grabar archivo con el reporte generado
    $pdf->Output('consulta_categorias_' . $fecha_reporte_archivo . '.pdf', 'D');
