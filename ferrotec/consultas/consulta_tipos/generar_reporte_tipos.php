<?php
/*
    PHP:            generar_reporte_tipos.php
    Descripción:    Arma el PDF con los resultados de la búsqueda de tipos
*/
    ...
    foreach($filas as $row){
        ...
        $tipo_descripcion = $row['descripcion'];
        $tipo_activo = $row['activo'];

        $html_filas .=
        "<tr>
            <td>{$tipo_nombre}</td>
            <td>{$tipo_descripcion}</td>
            <td>{$tipo_activo}</td>
        </tr>
        ";
    } 
    ...
    $pdf->SetTitle('Consulta de Tipos');
    $pdf->SetSubject('Consulta de Tipos');
    $pdf->SetKeywords('Ferrotec, Consulta, Tipos');
    ...
    "<html>
        <body>
            ...
            <br>
                <p style='float:left;'> <b> Consulta de Tipos </b> </p>
    ...
    header('Content-Disposition: attachment; filename="consulta_tipos' . $fecha_reporte_archivo . '.pdf"');
    header('Cache-Control: max-age=0');

    // Grabar archivo con el reporte generado
    $pdf->Output('consulta_tipos_' . $fecha_reporte_archivo . '.pdf', 'D');
